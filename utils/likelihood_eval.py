...
def compute_masked_likelihood(mu, data, mask, mu_gt=None, likelihood_func=None, temporal_weights=None): #时间mask
    ...
    if mask != None:
        log_prob_masked = torch.sum(log_prob * mask, dim=1)  # [n_traj, n_dims]，相乘代表mask掉了
        ...
        log_prob_masked_normalized = torch.div(log_prob_masked,
                                               timelength_per_nodes)  # 【n_traj, feature], average each feature by dividing time length，点除操作
